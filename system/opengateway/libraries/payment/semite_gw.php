...
class Semite_gw {
    ...
    */
    public function Settings()
    {
        ...
        $settings['name'] = 'Macropay';
        $settings['class_name'] = 'semite_gw';
        $settings['external'] = FALSE;
        $settings['no_credit_card'] = TRUE;
        $settings['description'] = 'Semite Payment makes it easy to start accepting credit cards on the web today. Requires a SSL connection.';
        $settings['is_preferred'] = 1;
        $settings['setup_fee'] = '$0';
        $settings['monthly_fee'] = '$0';
        $settings['transaction_fee'] = '2.9% + $0.30';
        $settings['purchase_link'] = 'http://processor.crm.io/v2/gateway/';
    ...
    */
    public function Charge($client_id, $order_id, $gateway, $customer, $amount, $credit_card, $return_url=null, $cancel_url=null, $custom=array())
    {
        ...
        // get customer country code iso
        $CI->db->where('order_id',$order_id);
        $order = $CI->db->get('orders')->row();

        $CI->db->where('customer_id',$order->customer_id);
        $customer = $CI->db->get('customers')->row();

        $CI->db->where('country_id',$customer->country);
    ...
    */
    public function Refund($client_id, $gateway, $charge, $authorization)
    {
        ...
        // get customer country code iso
        $CI->db->where('order_id',$authorization->order_id);
        $order = $CI->db->get('orders')->row();
        ...
        try
        {
            $operation = new Semite_BasicOperations_Refund();
            $operation->setMember($gateway['api_id'], $gateway['secret_key']);

            $operation->setTransactionId($authorization->tran_id);
            $operation->setTransactionGuid($authorization->authorization_code);

            $operation->setAmountAndCurrencyId((float)$order->amount,$gateway['currency']);

            $operation->setCountryCode($customer->country);

            $operation->setProcessor($gateway['processor']);

            $operation->setTrackingMemberCode('Refund ' . date('His dmY'));

            $client->call($operation);

            if ($operation->getResultCode() == 1) {
                return TRUE;
            } else {
                return FALSE;
            }
